...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below.
    ...
    The tidyverse package includes different tools for data analyzation and visualization with the goal of a tidy data organization.
...
levels_income_cat <- c("high income", "upper-middle income", "lower-middle income", "low income")
...
2.  Execute the code and observe the output in the Console. Would you publish this table in a report in this format?s

    no, we still have to make a prettier table
...
ggplot(data = waste_data_long_mean,
  ...
  labs(title = "Waste Composition",
       subtitle = "Mean percentages of nine wsate categories displayed by income categories",
       x = "mean (percent)",
       y = "waste category",
